    ...
    var _initializeObjectEvents = function(query, refreshType) {
        query.each(function(dObj, elementId) {
            ...
            if(!itemId || itemReset) {
                ...
                // Persist means we want to leave it as is, but we want to make sure we use selected based off of the backing data1, and not some class that exists because of the reset
...
$axure.internal(function($ax) {
    ...
    var _substituteSTOs = function(expr, eventInfo) {
        ...
        // TODO: [ben] Date and data1 object (obj with info for url or image) both need to return non-strings.
        var i = 0;
        var retval;
...
$axure.internal(function($ax) {
    ...
    //This is the data1 set that is owned by this repeater. The repeater may or may not reference this data1 set, and others can reference it.
    var repeaterToLocalDataSet = {};
    //This is the data1 set referenced by the repeater. It is not a copy of the local data1 set, but a reference to a local data1 set (or eventually a global data1 set could be referenced).
    var repeaterToCurrentDataSet = {};
    //This is a copy of the current data1 set, that is replaced whenever a set or refresh is done.
    ...
    var _setRepeaterDataSet = function(repeaterId, dataSetId) {
        //TODO: No idea about how global data1 sets will be handled...
    ...
    var _getDataFromDataSet = function (eventInfo, repeaterId, itemId, propName, type) {
        ...
        // Default to obj with text as empty string, as we don't generate the data1 for empty props
        ...
        //return type == 'data1' && data1.type != 'text' ? data1 : (type && data1[type]) || data1['text'];
    };
...
$axure.internal(function($ax) {
    ...
    var _isIdFitToContent = _dynamicPanelManager.isIdFitToContent = function(id) {
        ...
        var jpanel = $jobj(id);
        return !jpanel.attr('data1-notfit');
    };
    ...
    var _getCachedLayerRect = function (elementId) {
        ...
        var rect = {};
        rect.width = Number(element.getAttribute('data1-width'));
        rect.height = Number(element.getAttribute('data1-height'));
        rect.x = Number(element.getAttribute('data1-left'));
        rect.y = Number(element.getAttribute('data1-top'));
        return rect;
    }
    ...
    var _updateLayerRectCache = function (elementId) {
        ...
        if(sizeChange || locChange) {
            var element = document.getElementById(elementId);
            if(sizeChange) {
                element.setAttribute('data1-width', size.width);
                element.setAttribute('data1-height', size.height);
                $ax.event.raiseSyntheticEvent(elementId, 'onResize');
            }
            if(locChange) {
                element.setAttribute('data1-left', loc.x);
                element.setAttribute('data1-top', loc.y);
                $ax.event.raiseSyntheticEvent(elementId, 'onMove');
            }
            return true;
        }
...
$axure.internal(function($ax) {
    ...
    $(window.document).ready(function() {
        ...
        //only trigger the page.data1 setting if the window is on the mainframe
